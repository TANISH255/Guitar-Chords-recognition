...
from settings import CLASSES_MAP, MODEL_JSON, MODEL_H5, CLASSES, MODEL_DIR, LOG_DIR_TRAINING
...
class CNN(object):
    ...
    def evaluate(self, X_train, y_train, X_test, y_test):
        ...
        self.score_test = self.model.evaluate(x=X_test, y=y_test)
        self.score_train = self.model.evaluate(x=X_train, y=y_train)
    ...
    def predict(self, filepath, loadmodel=True):
        ...
        try:
            ...
            logger.info(f"Loaded audio file with shape: {y.shape}, sample rate: {sr}")
            ps = librosa.feature.melspectrogram(y=y, sr=sr)
            logger.info(f"Extracted spectrogram with shape: {ps.shape}")

            shape = (1,) + self.input_shape
            ps = np.array(ps.reshape(shape))
            logger.info(f"Reshaped spectrogram to: {ps.shape}")
